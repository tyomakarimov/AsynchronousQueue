...
class Queue {
    ...
    setFactors(string = 'ABC') {
        ...
        console.log(color.green + 'Factors of this Queue: ' + this.factorsOfQueue.join(', ') + color.white + '\n' + SEPARATOR) : 
        console.log(color.green + 'This Queue does not have factors' + color.white + '\n' + SEPARATOR);
        return this;
    }
    ...
    async start() {
        if (!this.pushed) throw new Error(color.red + 'Queue is empty. You must add the elements first!' + color.white);
        if (!this.formed) throw new Error(color.red + 'Queue has not been formed. You must use method init() !' + color.white);
        const waiting = [];
        for (let i = 0; i < this.queue.length; i++) {
            waiting.push(wait(this.timeouts[i] * 1000, 
            color.yellow + `${this.queue[i]} from user ${this.usersByTasks[i]}, waiting: ${this.queue.length - 1 - i}` + 
            color.white + '\n' + SEPARATOR ));
        };
...
const users = collectionOfUsers(map, 8);
